...
object OverlayStateJson {
    fun encode(state: OverlayState): String {
        ...
        // 格式：
        // x=0;y=200;alpha=1.0;oalpha=0.75;locked=1;collapsed=1;max=3;opponents=[id|name|R0Y0B0G0|ox|oy,...]
        ...
        return "x=${state.overlayX};y=${state.overlayY};alpha=${state.alpha};oalpha=${state.opponentAlpha};locked=${if (state.locked) 1 else 0};collapsed=${if (state.controlCollapsed) 1 else 0};max=${state.maxOpponents};opponents=[$opponents]"
    }
    ...
    private fun decode(raw: String): OverlayState {
        ...
        val opponentAlpha = map["oalpha"]?.toFloatOrNull() ?: 0.75f
        val locked = (map["locked"]?.trim() == "1")
        val controlCollapsed = map["collapsed"]?.trim()?.let { it == "1" } ?: true
        ...
        return OverlayState(
            ...
            opponentAlpha = opponentAlpha.coerceIn(0.4f, 1.0f),
            locked = locked,
